...
class ReplayMemory(object): 
    ...
    def push(self, *args):
        ...
        #index = self.index_no_reward if transition_.reward == 0 \
        #    else self.index_reward
        self.memory.add(transition_)

        if len(self.memory) >= self.capacity:
            #rm_data = self.sample(self.index_no_reward, self.num_remove)
    ...
    def __len__(self):
        #return len(self.memory[0]) + len(self.memory[1])
...
class ReplayMemoryL(object): 
    ...
    #    else:
    #        self.memory[self.next] = Transition(*args)
    ...
    def sample(self,  batch_size):
        return BatchSampler(SubsetRandomSampler(self.memory), batch_size, drop_last=False)
