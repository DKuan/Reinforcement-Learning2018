...
class RL_AGENT_ONE():
    ...
    def learn(self):
        ...
        train_cont = 0 
        for batch_data in self.memory.sample(self.batch_size):
                self.update_cont += 1
                s_o_set = []
                actions = []
                rewards = []
                ...
                if self.update_cont % self.update_period == 0:
                    self.target_net.load_state_dict(self.policy_net.state_dict())
            #for loss_ in loss:
            #    loss_.backward()
            #print('the fc_3 grad s is', self.policy_net.fc_3.weight.grad)
            #input()
            #print(self.policy_net.fc3_out)
            #for param in self.policy_net.parameters():
            #    param.grad.data.clamp_(-1, 1)
            #self.memory.update_priorities(idx_set, torch.tensor(loss).detach().numpy())


    def store_memory(self, env, step_now, obs_old, action, obs_new, reward, done):
        """
        This func is used to learn the agent
        par:
        |step_now: int, the global time of training
    ...
    def check_train(self, env):
        ...
        pass
        #for final_s in env.final_states:
        #    feature = self.feature_combine(final_s)
        #    print('the final state q_value is', self.policy_net(feature))
        #print('the fc_3 weight s is', self.policy_net.fc_3.weight)
